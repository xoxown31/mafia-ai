...
class RationalCharacter(BaseCharacter):
    ...
    def __init__(self, player_id: int, role: int = config.ROLE_CITIZEN):
        ...
        initial_belief = 25.0
        self.belief = np.full(
            (config.PLAYER_COUNT, 4), initial_belief, dtype=np.float32
        )
    ...
    def update_belief(self, game_status: dict):
        ...
        for claim in claims:
            ...
            self.memory.append(
                (current_day, speaker_id, reveal_role, target_id, assertion)
            )
            ...
            if target_id != -1:
                self._handle_assertion(
                    speaker_id, target_id, assertion, reveal_role, current_day
                )
        ...
        if execution_result:
            ...
            if len(execution_result) >= 4:
                ...
                self._process_execution(
                    (executed_id, team_alignment, day),
                    alive_players,
                    current_day,
                    vote_log,
                )
            ...
            for other_id, other_role in self.role_claims.items():
                if other_role == reveal_role and other_id != speaker_id:
                    ...
                    self._apply_belief_update(
                        speaker_id, config.ROLE_MAFIA, 30.0, "role_conflict"
                    )
                    self._apply_belief_update(
                        other_id, config.ROLE_MAFIA, 30.0, "role_conflict"
                    )

        self.role_claims[speaker_id] = reveal_role

        if reveal_role in [config.ROLE_POLICE, config.ROLE_DOCTOR]:
            self.trust_scores[speaker_id] = min(
                1.0, self.trust_scores.get(speaker_id, 0.5) + 0.2
            )
    ...
    def _handle_assertion(
        ...
        if assertion == "CONFIRMED_MAFIA":
            ...
            self._apply_belief_update(
                ...
                config.ROLE_MAFIA,
                -30.0 * trust_factor,
                "confirmed_citizen_claim",
            )
        elif assertion == "SUSPECT":
            self._apply_belief_update(
                target_id, config.ROLE_MAFIA, 15.0 * trust_factor, "suspect_claim"
            )

    def _handle_assertion_about_me(
        self, speaker_id: int, assertion: str, reveal_role: int
    ):
        ...
        if assertion == "CONFIRMED_CITIZEN" and self.role != config.ROLE_MAFIA:
            if reveal_role == config.ROLE_POLICE:
                ...
                self._apply_belief_update(
                    speaker_id, config.ROLE_POLICE, 80.0, "self_verification"
                )
                self._apply_belief_update(
                    speaker_id, config.ROLE_MAFIA, -100.0, "self_verification"
                )
        elif self.role != config.ROLE_MAFIA:
            self._apply_belief_update(
                speaker_id, config.ROLE_MAFIA, 15.0, "false_accusation"
            )
            self.trust_scores[speaker_id] = max(
                0.0, self.trust_scores.get(speaker_id, 0.5) - 0.2
            )
        elif self.role == config.ROLE_MAFIA:
            self._apply_belief_update(
                speaker_id, config.ROLE_POLICE, 20.0, "accused_me_as_mafia"
            )
    ...
    def _penalize_false_accusers(
        self, executed_id: int, day: int, alive_players: List[int]
    ):
        ...
        for accuser_id in false_accusers:
            if accuser_id != self.id and accuser_id in alive_players:
                ...
                self._apply_belief_update(
                    accuser_id, config.ROLE_MAFIA, 50.0, f"false_accuser_{executed_id}"
                )
    ...
    def _penalize_counter_claimers(self, executed_id: int, alive_players: List[int]):
        ...
        for other_id, other_role in self.role_claims.items():
            if (
                other_role == claimed_role
                and other_id != executed_id
                and other_id in alive_players
            ):
                self.trust_scores[other_id] = 0.0
                self._apply_belief_update(
                    other_id, config.ROLE_MAFIA, 80.0, "false_counter_claim"
                )
    ...
    def _reward_correct_accusers(self, executed_id: int, day: int):
        ...
        for accuser_id, accused_id, acc_day in self.accusation_history:
            if accused_id == executed_id and acc_day == day:
                self.trust_scores[accuser_id] = min(
                    1.0, self.trust_scores.get(accuser_id, 0.5) + 0.4
                )
    ...
    def _penalize_false_voters(
        self, executed_id: int, vote_log: Dict, alive_players: List[int]
    ):
        ...
        for voter_id in voters:
            if voter_id != self.id and voter_id in alive_players:
                ...
                self.trust_scores[voter_id] = max(
                    0.0, self.trust_scores.get(voter_id, 0.5) - 0.3
                )
                self._apply_belief_update(
                    voter_id,
                    config.ROLE_MAFIA,
    ...
    def _reward_correct_voters(self, executed_id: int, vote_log: Dict):
        ...
        for voter_id in voters:
            if voter_id != self.id:
                ...
                self.trust_scores[voter_id] = min(
                    1.0, self.trust_scores.get(voter_id, 0.5) + 0.3
                )
                self._apply_belief_update(
                    voter_id, config.ROLE_MAFIA, -40.0, f"voted_out_mafia_{executed_id}"
                )
                self._apply_belief_update(
                    voter_id, config.ROLE_POLICE, 15.0, f"voted_out_mafia_{executed_id}"
                )
    ...
    def _process_doctor_heal(self, night_result: Dict, game_status: dict):
        ...
        if no_death and last_healed != -1:
            ...
            self._apply_belief_update(
                last_healed, config.ROLE_CITIZEN, 15.0, "successful_heal"
            )
            self._apply_belief_update(
                last_healed, config.ROLE_MAFIA, -20.0, "successful_heal"
            )

    def _apply_belief_update(
        self, player_id: int, role_idx: int, delta: float, reason: str = ""
    ):
        """
    ...
    # ==========================================================================
    # [1] Discussion (토론): 무엇을 말할지 결정
    # ==========================================================================
    def decide_claim(
        self,
        players: List["BaseCharacter"],
        current_day: int = 1,
        discussion_context: List[Dict] = None,
    ) -> dict:
        ...
        if discussion_context:
            if self.role == config.ROLE_MAFIA:
                claim = self._check_mafia_counter_claim(
                    alive_ids, discussion_context, players
                )
            elif self.role == config.ROLE_DOCTOR:
                claim = self._check_doctor_super_save(
                    alive_ids, discussion_context, players
                )
    ...
    def _check_mafia_counter_claim(
        self,
        alive_ids: List[int],
        ...
        fellow_mafia = [
            p.id
            for p in players
            if p.role == config.ROLE_MAFIA and p.id != self.id and p.alive
        ]
        ...
        for claim in discussion_context:
            ...
            if (
                ...
                and assertion == "CONFIRMED_MAFIA"
                and (target_id == self.id or target_id in fellow_mafia)
                and speaker_id in alive_ids
            ):
    ...
    def _check_doctor_super_save(
        ...
        for healed_pid in self.healed_players:
            ...
            for claim in discussion_context:
                ...
                if target_id == healed_pid and assertion in [
                    "SUSPECT",
                    "CONFIRMED_MAFIA",
                ]:
            ...
            if (
                accusations_this_round >= 2
                or self.belief[healed_pid, config.ROLE_MAFIA] > 70.0
            ):
    ...
    def _police_claim_strategy(
        self, players: List["BaseCharacter"], alive_ids: List[int], current_day: int
    ) -> Dict:
        ...
        if alive_count <= 4:
            for pid in alive_ids:
                if (
                    pid in self.investigated_players
                    and self.belief[pid, config.ROLE_MAFIA] < -50
                ):
                    return {
                        "type": "CLAIM",
                        "reveal_role": config.ROLE_POLICE,
    ...
    def _doctor_claim_strategy(
        ...
        for healed_pid in self.healed_players:
            if healed_pid in alive_ids:
                ...
                accusations_against = sum(
                    1
                    for _, target, _ in self.accusation_history
                    if target == healed_pid
                )
    ...
    def _find_best_citizen_to_vouch(self, alive_ids: List[int]) -> int:
        ...
        citizen_scores = [
            (pid, -self.belief[pid, config.ROLE_MAFIA]) for pid in alive_ids
        ]
        citizen_scores.sort(key=lambda x: x[1], reverse=True)
        return citizen_scores[0][0] if citizen_scores else None

    def _claim_with_silence_threshold(
        self, alive_ids: List[int], current_day: int = 1
    ) -> Dict:
    ...
    def _mafia_claim_strategy(
        self, players: List["BaseCharacter"], alive_ids: List[int], current_day: int
    ) -> Dict:
        """Mafia claim strategy."""
        non_mafia_alive = [
            pid for pid in alive_ids if players[pid].role != config.ROLE_MAFIA
        ]
        ...
        police_suspects = [
            (pid, self.belief[pid, config.ROLE_POLICE]) for pid in non_mafia_alive
        ]
    ...
    def _citizen_vote_strategy(
        ...
        if alive_count <= 4:
            ...
            top_candidates = [
                pid for pid, score in mafia_scores if score == highest_score
            ]
            return np.random.choice(top_candidates)

        threshold = (
            60.0 * self.trust_threshold
            if current_day <= 2
            else 40.0 * self.trust_threshold
        )
    ...
    def _mafia_vote_strategy(
        self, players: List["BaseCharacter"], alive_ids: List[int]
    ) -> int:
    ...
    def _mafia_night_action(
        ...
        threat_scores = np.array(
            [
                (
                    100.0
                    if players[pid].should_reveal
                    and players[pid].role == config.ROLE_POLICE
                    else 0.0
                )
                + self.belief[pid, config.ROLE_POLICE] * 2.0
    ...
    def _doctor_night_action(
        ...
        return self._select_target_softmax(
            candidates, protection_scores, temperature=0.1
        )
    ...
    # =========================================================================
    # [NEW] Execution Vote Logic (처형 찬반 투표)
    # =========================================================================
    def decide_execution_vote(self, target_id: int) -> bool:
        """
        Decide whether to vote YES (execute) or NO (save) for the target.
        This logic mirrors the original game.py logic but now resides in the agent.
        """
        if target_id == self.id:
            return False  # Never vote to execute self

        # Check if I suspect this person as Mafia (Belief > 0)
        # Note: Game.py will initialize Mafia teammates with -100 belief,
        # so this logic automatically handles Mafia protecting teammates.
        return self.belief[target_id, config.ROLE_MAFIA] > 0
